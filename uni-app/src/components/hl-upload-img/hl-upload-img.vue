  ...
  methods: {
    ...
    chooseImg: function () {
      return new Promise((resolve, reject) => {
        ...
        UPD.selectImg(_option).then(({ tempFilePaths, tempFiles }) => {
          this.waitUploadImg = this.waitUploadImg.concat(tempFilePaths);
          this.localImages = this.localImages.concat(tempFilePaths);
          let _arr = this.waitUploadImg.filter(x => {
            return x != ''
          })
          let _data = {
            allImages: this.localImages,
            thisSelectImages: tempFilePaths,
